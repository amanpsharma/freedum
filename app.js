...
function showBar() {
  ...
  setMainPad($("topBar").offsetHeight);
...
$("scrollOverlay").addEventListener(
  "touchstart",
  (e) => {
...
const getLinks = () => {
  try {
    return JSON.parse(localStorage.getItem("fd_links") || "[]");
  } catch {
    return [];
  }
};
...
const isSaved = () =>
  currentUrl && getLinks().some((l) => l.url === currentUrl);
...
function saveLink() {
  ...
  if (links.find((l) => l.url === currentUrl)) {
    popIcon();
    return;
  }
  links.unshift({
    url: currentUrl,
    title: toTitle(currentUrl),
    at: Date.now(),
  });
  putLinks(links);
  renderLinks();
...
function renderLinks() {
  const links = getLinks();
  $("savedList").innerHTML = links.length
    ? links
        .map(
          (l, i) => `
...
window.addEventListener("resize", () => {
  if (barShown) setMainPad($("topBar").offsetHeight);
});
